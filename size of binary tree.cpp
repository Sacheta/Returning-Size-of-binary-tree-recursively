//Program generating a whole binary tree at run time. It implements the recursive sizeb(node*) function which returns the size of the binary tree passed to it via its root
#include<iostream>
#include<queue>
using namespace std;
...
int node::sizeb(node *root){  //recursive function for getting the size of tree
    if(root==0)  //if node is null
        return 0;
    return sizeb(root->left)+sizeb(root->right)+1;
}
...
int main(){
    ...
    cout<<"7. Print size of tree"<<endl;
    int n;
    while(cin>>n){
        switch(n){
        case 1:
        ...
        case 6:
            ...
            break;
        case 7:
            cout<<"Size is: "<<root->sizeb(root)<<endl;
            break;
        }
